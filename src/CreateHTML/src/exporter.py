...
def _message_to_html(m: Message) -> str:
    ...
    content = m.get_content() if m.media_wa_type != '0' else _esc(m.get_content())

    if m.remote_resource:
        return f"<div class='message {direction_class}'><div class='sender'>{_esc(m.get_sender_name())}</div>" \
               f"{content}<div class='time'>{_esc(m.received_timestamp_str)}</div></div>"
    else:
        return f"<div class='message {direction_class}'>{content}" \
...
def _save_to_html_file(chat_contents: str, chats_list: str, filepath: str):
    ...
    js_code = _load_file_content(".\\CreateHTML\\src\\res\\main.js")
    css_code = _load_file_content(".\\CreateHTML\\src\\res\\styles.css")
    # Avoid SyntaxError: f-string must not include a backslash
    template = _load_file_content('.\\CreateHTML\\src\\res\\template.html').replace("\n", "")
