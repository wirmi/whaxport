...
def query_messages(con: Connection, key_remote_jid: str, contacts: Dict[str, Optional[str]]) -> List[Message]:
    cur = con.cursor()
    query = """
            SELECT _id, timestamp, remote_resource, key_from_me, data, media_caption, media_wa_type, media_hash 
            FROM messages 
    ...
    for _id, timestamp, remote_resource, key_from_me, data, media_caption, media_wa_type, media_hash in execute:
        ...
        messages.append(
            Message(timestamp, remote_resource, key_from_me, data, media_caption, media_wa_type, contacts.get(remote_resource, None), media_path)
        )
    return messages
